...
def decode_td3_mrz_lines(line1: str, line2: str):
    ...
    try:
        ...
        # Format dates
        birth_date_formatted = f"20{birth_date[0:2]}-{birth_date[2:4]}-{birth_date[4:6]}" if len(birth_date) == 6 else birth_date
        expiry_date_formatted = f"20{expiry_date[0:2]}-{expiry_date[2:4]}-{expiry_date[4:6]}" if len(expiry_date) == 6 else expiry_date
